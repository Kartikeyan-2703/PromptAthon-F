  ...
  return <PortalLayout admin><div className="portal-content">
    ...
    {round === 2 ? <RoundTwoEvaluationQueue /> : <>
      ...
      {error ? <div className="portal-error"><X size={15} />{error}</div> : loading ? <div className="portal-loading"><LoaderCircle className="spin" />Loading database records…</div> : <>
        <section className="submission-admin-list">{data?.items.map((item) => <article key={item.id}><div><StatusBadge status={displayStatus(item.status)} /><span>{item.hasSubmission === false ? 'RESULT' : item.id.slice(0, 8)}</span></div><h2>{item.team.code}</h2><dl><div><dt>ROUND</dt><dd>Round 0{item.round.number}</dd></div><div><dt>SUBMITTED</dt><dd>{time(item.submittedAt)}</dd></div><div><dt>EVIDENCE</dt><dd>{item.hasSubmission === false ? 'No submission record' : `${item._count.answers} answers`}</dd></div></dl>{item.hasSubmission === false ? <span className="button secondary" aria-disabled="true">Final result</span> : <Link className="button secondary" href={`/admin/submissions/${item.id}`}>Review <ArrowRight /></Link>}</article>)}{!data?.items.length && <EmptyState title="NO SUBMISSIONS" message="No submitted evidence matches this round and status." />}</section>
