...
function PrizeSection(){ return <motion.section className="prize-section" {...reveal}><p className="eyebrow"><span>05 / </span>PRIZE SYSTEM</p><div className="prize-stage"><Trophy size={22}/>{[['02','₹2000','SECOND PLACE'],['01','₹3000','FIRST PLACE'],['03','₹1000','THIRD PLACE']].map(([n,p,l])=><div className={`prize prize-${n}`} key={n}><span>{n}</span><strong>{p}</strong><p>{l}</p></div>)}</div></motion.section>; }
function FinalCta(){ return <section className="final-cta"><div className="scanline"><ScanLine/></div><p className="eyebrow">&gt; AWAITING INPUT_</p><h2>THINK LIKE A HACKER.<br/><span>PROMPT LIKE A PRO.</span></h2><p>Your challenge begins with a blank input.</p><Link className="primary-button" href="/login">Participant login <ArrowRight size={17}/></Link></section>; }
